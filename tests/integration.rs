...
mod integration_tests {
    ...
    fn validate_result(result: &QueryResult, expected_col_count: i32, min_row_count: i32) {
        ...
        if expected_col_count > 0 {
            ...
            assert_eq!(
                result.col_count, expected_col_count,
                "Column count mismatch"
            );
            assert_eq!(
                result.column_names.len() as i32,
                expected_col_count,
        ...
        assert!(
            result.row_count >= min_row_count,
            "Row count should be at least {}",
            min_row_count
        );
    }
    ...
    fn validate_row_has_columns(row: &HashMap<String, Value>, expected_columns: &[&str]) {
        for col in expected_columns {
            assert!(
                row.contains_key(&col.to_string()),
    ...
    fn test_registry_pull() {
        ...
        let result = conn
            .query("REGISTRY PULL homebrew")
            .expect("REGISTRY PULL should succeed");
    ...
    fn test_simple_select_no_rows() {
        ...
        let result = conn
            .query("SELECT 1 as col_name WHERE 1=0")
            .expect("Simple SELECT with no rows should succeed");
    ...
    fn test_failed_command() {
        ...
        assert!(
            ...
            "Error should contain 'syntax error'"
        );
    }
    ...
    fn test_stackql_select_after_error() {
        ...
        assert!(
            result
                .column_names
                .iter()
                .all(|col| expected_columns.contains(&col.as_str())),
        ...
        if !result.rows.is_empty() {
            ...
            assert_eq!(
                row.get("formula_name").unwrap().to_string(),
                "stackql",
                "Formula name should be stackql"
            );
        }
    }
    ...
    fn test_stackql_select_with_provider_error() {
        ...
        println!(
            "\n{}",
            "StackQL SELECT example with provider error and no rows"
                .blue()
                .bold()
        );
        ...
        assert!(
            result.notice_count > 0,
            "Should have notices for provider error"
        );
