...
// tests/integration.rs
// Integration tests for the PgwireLite library against a server without TLS
...
// Setup function to create a connection without TLS
fn setup() -> PgwireLite {
    PgwireLite::new(SERVER_HOST, SERVER_PORT, false, "verbose")
        .expect("Failed to create connection to non-TLS server")
}

// Test for setting up a connection with TLS but against a non-TLS server (should fail)
// #[test]
// fn test_tls_connection_to_non_tls_server_fails() {
//     let conn_result = PgwireLite::new(SERVER_HOST, SERVER_PORT, true, "verbose");
//     assert!(conn_result.is_err(), "TLS connection to non-TLS server should fail");
//     let err = format!("{}", conn_result.err().unwrap());
//     assert!(
//         err.contains("connection to server") && 
//         (err.contains("failed") || err.contains("refused") || err.contains("closed")),
//         "Expected connection error, got: {}", err
//     );
// }

// Run all the shared test cases against a non-TLS server
#[test]
fn test_libpq_version_non_tls() {
    let conn = setup();
    test_cases::test_libpq_version(&conn);
}

#[test]
fn test_verbosity_non_tls() {
    let conn = setup();
    test_cases::test_verbosity(&conn);
}

#[test]
fn test_simple_query_one_row_non_tls() {
    let conn = setup();
    test_cases::test_simple_query_one_row(&conn);
}

#[test]
fn test_simple_query_no_rows_non_tls() {
    let conn = setup();
    test_cases::test_simple_query_no_rows(&conn);
}

#[test]
fn test_registry_list_non_tls() {
    let conn = setup();
    test_cases::test_registry_list(&conn);
}

#[test]
fn test_registry_pull_homebrew_non_tls() {
    let conn = setup();
    test_cases::test_registry_pull_homebrew(&conn);
}

#[test]
fn test_registry_pull_github_non_tls() {
    let conn = setup();
    test_cases::test_registry_pull_github(&conn);
}

#[test]
fn test_invalid_command_non_tls() {
    let conn = setup();
    test_cases::test_invalid_command(&conn);
}

#[test]
fn test_stackql_multiple_rows_non_tls() {
    let conn = setup();
    test_cases::test_stackql_multiple_rows(&conn);
}

#[test]
fn test_stackql_provider_error_non_tls() {
    let conn = setup();
    test_cases::test_stackql_provider_error(&conn);
}

#[test]
fn test_stackql_specific_repo_non_tls() {
    let conn = setup();
